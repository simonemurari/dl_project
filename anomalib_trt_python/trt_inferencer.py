...
class TrtInferencer():
    ...
    def __init__(
        ...
        if self.dynamic_batch:
            _ = self.context.set_input_shape(input_name, (self.batch_size, 3, self.input_h, self.input_w))
    ...
    def predict(self, image, metadata=None):
        ...
        trt_outputs = common.do_inference_v3(engine=self.engine, context=self.context, bindings=self.bindings, inputs=self.inputs, outputs=self.outputs, stream=self.stream)
        ...
        return ImageResult(
            image=(image_arr * 255).astype(np.uint8),
...
if __name__ == "__main__":
    ...
    visualizer = ImageVisualizer(mode='full', task='segmentation')
